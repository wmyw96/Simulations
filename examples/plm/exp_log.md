...
Experiment `1.7.7`, stored across the simulation artifacts `1.7_7`, `1.7_7_tracking`, and `1.7_7_minimax`.

### Goal

This experiment revisits the same projected tanh-wrapped design as `1.7.5`, but increases the Monte Carlo budget from `10` to `30` trials and now interprets the sample-size parameter as the per-split size. The goal is to stabilize the comparison across the learned estimators in the larger-sample regime and to bundle the full three-part diagnostic package in one place:

- the final estimator comparison across Oracle AIPW, validation-selected DML, standard DML, and minimax debias;
- the DML nuisance-learning curve over epochs; and
- the minimax beta-path ablation over epochs.
...
- DML nuisance-path diagnostic:
  - artifact `1.7_7_tracking`
  - oracle validation sample size `2048`
  - averaged validation oracle `mu` and `pi` MSE paths on epochs `0, 10, ..., 200`
- Minimax ablation diagnostic:
  - artifact `1.7_7_minimax`
  - oracle validation sample size `2048`
  - final minimax weights paired with saved `mu` checkpoints on epochs `0, 10, ..., 200`
...
| pi family | Oracle AIPW beta MSE | Validation-selected DML beta MSE | Standard DML beta MSE | Minimax debias beta MSE |
| --- | ---: | ---: | ---: | ---: |
| `r = 1` | 0.000701 | 0.002359 | 0.004848 | 0.001182 |
| `r = 2` | 0.000701 | 0.002800 | 0.002515 | 0.001382 |
| `r = 4` | 0.000702 | 0.002312 | 0.003118 | 0.001712 |
| `r = 8` | 0.000703 | 0.002604 | 0.006370 | 0.001825 |

Nuisance MSEs for the two Neural DML variants:

| pi family | Selected DML mu MSE | Selected DML pi MSE | Standard DML mu MSE | Standard DML pi MSE |
| --- | ---: | ---: | ---: | ---: |
| `r = 1` | 0.079437 | 0.092618 | 0.202617 | 0.332972 |
| `r = 2` | 0.076715 | 0.174514 | 0.299684 | 0.261055 |
| `r = 4` | 0.073149 | 0.219692 | 0.209786 | 0.339374 |
| `r = 8` | 0.072764 | 0.250427 | 0.175632 | 0.425520 |

Bias-variance decomposition of beta estimation error:

| pi family | Selected DML squared bias | Selected DML variance | Standard DML squared bias | Standard DML variance | Minimax squared bias | Minimax variance |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `r = 1` | 0.001581 | 0.000778 | 0.000428 | 0.004420 | 0.000561 | 0.000620 |
| `r = 2` | 0.001812 | 0.000988 | 0.001134 | 0.001381 | 0.000808 | 0.000574 |
| `r = 4` | 0.001565 | 0.000747 | 0.000533 | 0.002585 | 0.001137 | 0.000575 |
| `r = 8` | 0.001482 | 0.001122 | 0.000200 | 0.006170 | 0.001325 | 0.000500 |
...
DML nuisance-path summary from `1.7_7_tracking`:
...
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| `r = 1` | 0.480717 | 0.073784 | 52 | 0.442291 | 0.087286 | 54 |
| `r = 2` | 0.480717 | 0.073244 | 52 | 0.405122 | 0.168706 | 70 |
| `r = 4` | 0.480717 | 0.072402 | 52 | 0.396858 | 0.227720 | 72 |
| `r = 8` | 0.480717 | 0.072076 | 52 | 0.396295 | 0.260159 | 71 |

Minimax ablation summary from `1.7_7_minimax`:
...
| --- | ---: | ---: | ---: | ---: | ---: |
| `r = 1` | 0.075211 | 50 | 0.001021 | 120 | 0.001182 |
| `r = 2` | 0.074081 | 50 | 0.001321 | 150 | 0.001382 |
| `r = 4` | 0.072853 | 50 | 0.001545 | 170 | 0.001712 |
| `r = 8` | 0.072411 | 50 | 0.001626 | 160 | 0.001825 |

Main observations:

- Moving from the old total-size convention to the per-split convention substantially helps every method because the total training sample size doubles from `2048` to `4096`. Oracle beta MSE drops from about `9.5e-4` to about `7.0e-4`, and all learned estimators improve as well.
- Minimax is now the strongest learned method across all four `r` values, with beta MSE between about `1.18e-3` and `1.82e-3`. Validation-selected DML comes next, while standard DML remains more unstable and now shows its best performance only around `r = 2`.
- Validation-selected DML still dominates nuisance estimation. Its `mu` MSE is now in the `0.072` to `0.079` range and its `pi` MSE ranges from about `0.093` to `0.250`, both markedly better than in the smaller-sample total-size version.
- The DML nuisance tracker shows that the larger effective sample size shifts the average validation minima later: the average `mu` path bottoms out around epoch `52` for all four `r` values, while the average `pi` path bottoms out between epochs `54` and `72`.
- The minimax beta path again prefers later checkpoints than the nuisance path. In this larger-sample regime the best minimax beta MSE occurs between epochs `120` and `170`, noticeably later than the `mu` minima around epoch `50`.

Generated figures:

- `examples/plm/figs/1.7/1.7.7_pi_complexity_mean_mse_comparison.png`
- `examples/plm/figs/1.7/1.7.7_pi_complexity_beta_bias_sq.png`
- `examples/plm/figs/1.7/1.7.7_pi_complexity_beta_variance.png`
