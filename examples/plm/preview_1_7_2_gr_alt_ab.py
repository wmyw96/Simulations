"""Preview the fixed-coefficient Fourier family g_r with alternating sine and cosine signs."""
...
def evaluate_g_r(x: np.ndarray, *, r: float, num_terms: int = NUM_TERMS) -> np.ndarray:
    """Evaluate the normalized Fourier family with a_k = b_k = (-1)^k."""
...
def main() -> None:
    ...
    axis.set_title(r"Preview of the Fourier family $g_r(x)$ with $a_k=b_k=(-1)^k$")
    axis.legend(loc="upper right")
    axis.grid(alpha=0.18, linewidth=0.6)
    figure.tight_layout()

    output_path = fig_dir / "1.7.2_g_r_preview_alt_ab.png"
    figure.savefig(output_path, dpi=220)
    plt.close(figure)

    metadata_path = fig_dir / "1.7.2_g_r_preview_alt_ab_metadata.json"
    metadata = {
        "r_values": list(R_VALUES),
        "num_terms": NUM_TERMS,
        "coefficients": {"a_k": "(-1)^k", "b_k": "(-1)^k"},
