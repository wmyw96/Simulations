"""Preview the fixed-coefficient Fourier family g_r for the planned PLM 1.7.2 study."""
...
def evaluate_g_r(x: np.ndarray, *, r: float, num_terms: int = NUM_TERMS) -> np.ndarray:
    """Evaluate the normalized fixed-coefficient Fourier family g_r on a grid."""
    ...
    denominator = np.sqrt(np.sum(2.0 * (weights**2)))
    if denominator <= 0:
...
def main() -> None:
    ...
    axis.set_title(r"Preview of the fixed-coefficient Fourier family $g_r(x)$")
    axis.legend(loc="upper right")
    axis.grid(alpha=0.18, linewidth=0.6)
    figure.tight_layout()

    output_path = fig_dir / "1.7.2_g_r_preview.png"
    figure.savefig(output_path, dpi=220)
    plt.close(figure)

    metadata_path = fig_dir / "1.7.2_g_r_preview_metadata.json"
    metadata = {
        "r_values": list(R_VALUES),
        "num_terms": NUM_TERMS,
        "coefficients": {"a_k": 1.0, "b_k": 1.0},
        "normalization": "sqrt(sum_{k=1}^{10} (a_k^2 + b_k^2) k^(-2/r)) with a_k = b_k = 1",
    }
